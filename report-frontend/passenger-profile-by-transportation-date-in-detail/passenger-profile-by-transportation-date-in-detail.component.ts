...
@Component({
  selector: 'app-passenger-profile-by-transportation-date-in-detail',
  templateUrl: './passenger-profile-by-transportation-date-in-detail.component.html',
  styleUrls: ['./passenger-profile-by-transportation-date-in-detail.component.scss'],
})
export class PassengerProfileByTransportationDateInDetailComponent {
  form = new FormGroup({});
  model = {};
  ...
  onSubmit() {
    if (this.form.valid) {
      // @ts-ignore
      const {   startDate, endDate ,riskLevel} = this.model;


      // Perform your logic here, e.g., call the API with the selected values
      this.loadReport( startDate, endDate,riskLevel);
    }
  }
  ...
  // @ts-ignore
  loadReport(  startDate, endDate,riskLevel): void {
    console.log(status, startDate, endDate )
    const headers = new HttpHeaders({ 'Accept': 'application/pdf' });
    this.http.get(environment.cmsGateWayRootContext + 'customsOperations/passenger/report/transportationDate/inDetail/'  + startDate + '/' + endDate +'/'+riskLevel, {
      headers: headers,
