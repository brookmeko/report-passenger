...
@Component({
  selector: 'app-passenger-profiles-by-frequency-of-each-criteria',
  templateUrl: './passenger-profiles-by-frequency-of-each-criteria.component.html',
  styleUrls: ['./passenger-profiles-by-frequency-of-each-criteria.component.scss'],
})
export class PassengerProfilesByFrequencyOfEachCriteriaComponent {
  form = new FormGroup({});
  model = {};
  ...
  loadReport(  startDate, endDate,reportFormat): void {
    ...
    this.http.get(environment.cmsGateWayRootContext + 'customsOperations/passenger/report/riskLevel/frequencyOfEachCriteria/'  + startDate + '/' + endDate  +'/'+ reportFormat, {
      headers: headers,
