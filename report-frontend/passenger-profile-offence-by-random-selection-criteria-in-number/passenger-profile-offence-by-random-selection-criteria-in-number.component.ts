...
@Component({
  selector: 'app-passenger-profile-offence-by-random-selection-criteria-in-number',
  templateUrl: './passenger-profile-offence-by-random-selection-criteria-in-number.component.html',
  styleUrls: ['./passenger-profile-offence-by-random-selection-criteria-in-number.component.scss'],
})
export class PassengerProfileOffenceByRandomSelectionCriteriaInNumberComponent {
  form = new FormGroup({});
  model = {};
  ...
  onSubmit() {
    if (this.form.valid) {
      // @ts-ignore
      const {   startDate, endDate ,reportFormat, transportationNumber} = this.model;


      // Perform your logic here, e.g., call the API with the selected values
      this.loadReport(  startDate, endDate,reportFormat, transportationNumber);
    }
  }
  ...
  // @ts-ignore
  loadReport(  startDate, endDate,reportFormat,transportationNumber): void {
    console.log(status, startDate, endDate )
    const headers = new HttpHeaders({ 'Accept': 'application/pdf' });
    this.http.get(environment.cmsGateWayRootContext + 'customsOperations/passenger/report/randomSelection/offence/transportationNumber/inNumber/'  + startDate + '/' + endDate + '/' + transportationNumber +'/'+ reportFormat, {
      headers: headers,
