...
@Component({
  selector: 'app-passenger-profile-by-risk-level-frequency-in-number',
  templateUrl: './passenger-profile-by-risk-level-frequency-in-number.component.html',
  styleUrls: ['./passenger-profile-by-risk-level-frequency-in-number.component.scss'],
})
export class PassengerProfilesByRiskLevelFrequencyInNumberComponent {
  form = new FormGroup({});
  model = {};
  ...
  loadReport(  startDate, endDate,reportFormat): void {
    ...
    this.http.get(environment.cmsGateWayRootContext + 'customsOperations/passenger/report/riskLevel/frequency/inNumber/'  + startDate + '/' + endDate +'/'+reportFormat, {
      headers: headers,
