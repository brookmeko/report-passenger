...
@Component({
  selector: 'app-passenger-profiles-rerouted-random-selection-risk-level',
  templateUrl: './passenger-profiles-rerouted-random-selection-risk-level.component.html',
  styleUrls: ['./passenger-profiles-rerouted-random-selection-risk-level.component.scss'],
})
export class PassengerProfilesReroutedRandomSelectionRiskLevelComponent {
  form = new FormGroup({});
  model = {};
  ...
  loadReport(  startDate, endDate,reportFormat): void {
    ...
    this.http.get(environment.cmsGateWayRootContext + 'customsOperations/passenger/report/rerouted/randomSelection/'  + startDate + '/' + endDate  +'/'+ reportFormat, {
      headers: headers,
